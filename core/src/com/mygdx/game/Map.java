...
public class Map {
    public enum WallType{
        HARD(0, 5, true), SOFT(1, 3, true), DESTRUCTIBLE(2, 1, false), NONE(0,0,false);
        int index;
        int maxHp;
    ...
    public Map(TextureAtlas atlas) {
        ...
        for (int i = 0; i < SIZE_X; i++) {
            cells[i][0].changeType(WallType.DESTRUCTIBLE);
            cells[i][SIZE_Y - 1].changeType(WallType.DESTRUCTIBLE);

        }
        for (int i = 0; i < SIZE_Y; i++) {
            cells[0][i].changeType(WallType.DESTRUCTIBLE);
            cells[SIZE_X - 1][i].changeType(WallType.DESTRUCTIBLE);
